...
conda create -n $ENV_NAME python=3.12 --yes -c defaults
...
MAX_JOBS=48 PYTORCH_ROCM_ARCH='gfx942' GPU_ARCHS='gfx942' pip install --pre torch==2.8.0.dev20250502+rocm6.4 --index-url https://download.pytorch.org/whl/nightly/rocm6.4

pip install ninja packaging numpy

cd ${INSTALLDIR}
git clone https://github.com/ROCm/xformers.git xformers_lingua
cd xformers_lingua
git submodule update --init --recursive
MAX_JOBS=48 PYTORCH_ROCM_ARCH=gfx942 python setup.py install
cd ${INSTALLDIR}
rm -rf xformers_lingua
